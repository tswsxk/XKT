...
__all__ = ["extract", "bucket_transform", "transform", "etl"]


def bucket_transform(raw_data, params):
    # 定义数据转换接口
    # raw_data --> batch_data
...
def etl(data_src, params, bucket=True):
    if not bucket:
        raw_data = extract_iter(data_src)
        return transform(raw_data, params)
    else:
        raw_data = extract(data_src)
        return bucket_transform(raw_data, params)


def pseudo_data_iter(_cfg):
    return bucket_transform(pseudo_data_generation(_cfg), _cfg)
